...
import { DMChannel, PermissionFlagsBits } from 'discord.js'
...
emoji_default.sort((a, b) => b.length - a.length)
...
async function pollChannel (message) {
  if (
    ...
    message.channel.lastMessageId === void 0
  ) {
...
var getSourceFlipped = async (message, [channelId, messageId]) =>
...
var getReplyFlipped = async (message, [channelId, messageId]) =>
...
var welcomeChannels = new CachedMap('./data/welcome-channels.json')
var sentienceMsgSent = new CachedMap('./data/sentience-msg-sent.json')
var onReady2 = () =>
  Promise.all([welcomeChannels.read(), sentienceMsgSent.read()])
async function setWelcome (message, [channelId, welcomeMsg]) {
  if (
    message.channel instanceof DMChannel2 ||
    message.channel.lastMessageId === void 0
  ) {
  ...
  if (
    !message.channel
      .permissionsFor(message.member)
      .has(PermissionFlagsBits2.ManageGuild)
  ) {
    await message.reply(
...
async function setLockdownCategory (message, [categoryId]) {
  if (
    message.channel instanceof DMChannel3 ||
    message.channel.lastMessageId === void 0
  ) {
    await message.reply('no dms')
    return
  ...
  if (
    !message.channel
      .permissionsFor(message.member)
      .has(PermissionFlagsBits3.ManageChannels)
  ) {
    await message.reply(
      ...
      embeds: [
        {
          ...
          } ([link to message](${lastMention.url})):
  ...
  if (user) {
    user
      .send(
        `You were warmed in [${message.guild?.name ?? 'DMs'}](${
          ...
          message.author.id
        }> thought you needed warmth. \u{1F970}`
      )
      .catch(() => {})
    await message.reply({
      embeds: [
        {
          color: 16612098,
          description: `\u{1F970} <@${userId}> has been warmed.`
        }
      ]
    })
  } else {
    await message.reply({
      embeds: [
        {
          color: 15286850,
          description: `\u{1F614} Couldn't warm <@${userId}>.`
        }
      ]
    })
  }
...
async function serverStatus (message, [address]) {
  if (!address) {
    ...
    if (!info) {
      await message.reply({
        content:
          'idk what address u want. i default to whatever you set `track:` to but it looks like you arent using that so \u{1F937}'
      })
      return
    }
  ...
  const embeds = start
    ...
    : [
      ...
      ...players
        ...
        .map(createEmbed(' joined the game.', 2278750)),
      // Left
      ...state.lastPlayers
        ?.filter(({ id }) => !players.some(p => p.id === id))
        .map(createEmbed(' left the game.', 15680580))
    ]
...
import { PermissionFlagsBits as PermissionFlagsBits4 } from 'discord.js'
async function isMenu (message) {
  ...
  return message.guild.members
    ...
    .then(member => member.permissions.has(PermissionFlagsBits4.ManageRoles))
    .catch(() => false)
}
...
async function notify (
  client2,
  message,
  { color = 'info', pingOwner = false, file, footer } = {}
) {
...
var FreeFoodScraper = class {
  ...
  async #handleGraphQlResponse (response) {
    ...
    if (storyData) {
      const userStories = storyData.edges.map(({ node: user }) => ({
    ...
    try {
      ...
      for (const script of await page
        ...
        const json = await script
          .textContent()
          .then(json2 => json2 && JSON.parse(json2))
          .catch(() => {})
...
async function scrapeFreeFood (client2, nextTime) {
  ...
  const getFooter = () =>
    `Took ${((performance.now() - start) / 1e3).toFixed(
      3
    ...
    embeds: [
      {
        ...
        fields: Array.from(aliases.values(), ([name, ...aliases2]) => ({
          name,
          value: aliases2.length
            ? `or ${aliases2.map(alias => `\`${alias}\``).join(' or ')}`
            : select([
              'no aliases, nice',
              "that's it",
...
var commands = {
  ...
  'get raw message source of message in channel <id> with id <id>':
    source_exports.getSourceFlipped,
  ...
  'get message that message in channel <id> with id <id> replied to':
    source_exports.getReplyFlipped,
  ...
  'turn on poll channel mode which auto-adds reactions to messages':
    poll_reactions_exports.pollChannel,
  ...
  "deny the unverified access to the commoners' channels":
    vote_lockdown_exports.voteLockdown,
...
async function handleMessage (message) {
  if (ignore_us_exports.ignoreState.endPhrase !== null) {
    if (
      message.author.id === process.env.OWNER &&
      message.content === ignore_us_exports.ignoreState.endPhrase
    ) {
  ...
  const reactions =
    poll_reactions_exports.getReactions(message, true) ??
    (await reaction_roles_exports.getReactions(message))
  if (reactions) {
...
client.on('messageUpdate', async (_oldMessage, newMessage) => {
  ...
  const reactions =
    poll_reactions_exports.getReactions(newMessage, false) ??
    (await reaction_roles_exports.getReactions(newMessage))
  if (reactions) {
...
var printTime = () =>
  /* @__PURE__ */ new Date().toLocaleString('ja-JP', {
    timeZone: 'America/Los_Angeles'
  })
try {
...
fs3.writeFileSync('./data/last_pid.txt', `[${printTime()}] ${process.pid}`)
