...
const printTime = () => new Date().toLocaleString('ja-JP', {
...
try {
    ...
    try {
        fs.writeFileSync('./data/EventLoggerError.txt', `[${printTime()}] ${error instanceof Error ? error.stack || error.message : error}`);
    }
    catch { }
}
